...
public class YoungTableau {
    ...
    public static void minHeapify(int[][] matrix, int i, int j) {
        ...
        while (true) {
            ...
            AlgoUtil.swapInMatrix(matrix, i, j, smallRow, smallColumn);
            i = smallRow;
            j = smallColumn;
        }
    ...
    public static void maxHeapify(int[][] matrix, int i, int j) {
        while (true) {
            ...
            AlgoUtil.swapInMatrix(matrix, i, j, largestRow, largestColumn);
            i = largestRow;
            j = largestColumn;
        }
    ...
    public static boolean isYoungTableau(int[][] matrix) {
        ...
        for (int i = 0; i < matrix.length; i++) {
            if (!AlgoUtil.isOrdered(matrix[i])) {
                flag = false;
            }
        }
        AlgoUtil.transformMatrix(matrix);
        for (int i = 0; i < matrix.length; i++) {
            if (!AlgoUtil.isOrdered(matrix[i])) {
                flag = false;
            }
