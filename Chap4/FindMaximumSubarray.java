...
public class FindMaximumSubarray {
    ...
    public static int findMaximumSubarrayBF(int[] arr) {
        ...
        for (int i = 0; i < arr.length; i++) {
            for (int j = i; j < arr.length; j++) {
                sum = Math.max(SimpleUtil.calculateSubarraySum(arr, i, j), sum);
    ...
    public static int determineMaxNWhenBFBetter(int maxScale) {
        int[] arr = SimpleUtil.generateRandomArray(maxScale, 5, -5);
    ...
    public static int advancedFindMaximumSubarray(int[] arr) {
        ...
        while (length >= 0) {
            if (arr[length] >= 0) {
                sum = Math.max(sum, SimpleUtil.getMaxSumIncludedLastElement(arr, length));
            }
            length = length - 1;
        }
    ...
    public static int advanced2FindMaximumSubarray(int[] arr) {
        ...
        for (int i = 0; i < arr.length; i++) {
            if (arr[i] >= 0) {
                sum = Math.max(sum, SimpleUtil.getMaxSumIncludedLastElement(arr, i));
            }
        }
        return sum;
    }
    ...
    public static void testForFindMaximumSubarray(int maxTestTimes) {
        for (int i = 0; i < maxTestTimes; i++) {
            int[] arr = SimpleUtil.generateRandomArray(5, 5, -3);
